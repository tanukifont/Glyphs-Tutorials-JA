...
*   _Color > Add sbix Images to Font:_ フォルダ内のすべてのPNG、GIF、JPGファイルを取得し、現在のフォントとマスターにそれらを持つ_iColor_レイヤーを作成します。スクリプトは次のファイル命名規則を期待します：`グリフ名 ピクセルサイズ.拡張子`、例：`Adieresis 128.png`。
